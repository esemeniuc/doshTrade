...
function BellButton({ticker}: { ticker: string }) {
    ...
    if (userConsent === 'default') {
        return (
            <IconButton className={classes.mutedButton} onClick={() => {
                dispatch(pushPermissionRequest())
            }}>
                <NotificationsNone/>
            </IconButton>)
    } else if (userConsent === 'denied') {
        return (<IconButton className={classes.mutedButton} onClick={() => {
            alert("push permission is denied")
        }}>
            <NotificationsOff/>
        </IconButton>)
    } else if (!subscription) {
        return (<IconButton className={classes.activeButton} onClick={() => {
            dispatch(pushPermissionRequest())
        }}>
            <NotificationsNone/>
        </IconButton>)
